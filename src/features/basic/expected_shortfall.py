...
#
# Two outputs:
#   1) Rolling ES per row → written back into data/features/*.parquet (Layer 3)
#   2) Snapshot (latest value per ticker) → data/risk/expected_shortfall.parquet (Layer 6)
#
# New columns added to feature parquets:
...
#
# Requires: 'returns' column — must run after run_returns().
